    ...
    def forward(
        ...
        **kwargs
        ...
        if layer_idx == 0:
            ...
            torch.save(tensors, f'/root/theo_db/projets/Decoder2Encoder/data_test/ref_attention_layer{self.layer_idx}.pt')
            
            torch.save(hidden_states.cpu(), f'/root/theo_db/projets/Decoder2Encoder/data_test/ref_hidden_states.pt')
            torch.save(attention_mask.cpu(), '/root/theo_db/projets/Decoder2Encoder/data_test/ref_attention_mask_layer0.pt')
...
class Qwen2_5OmniDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs
        ...
        #}
#
        #torch.save(tensors, f'/root/theo_db/projets/Decoder2Encoder/data_test/ref1_layer{self.layer_idx}.pt')
...
class Qwen2_5OmniThinkerTextModel(Qwen2_5OmniPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        #normalized_hidden_states = normalize_tensor_shape(hidden_states, attention_mask)
        #normalized_position_ids = normalize_tensor_shape(position_ids, attention_mask)
...
#
        #}
#
        #torch.save(tensors, '/root/theo_db/projets/Decoder2Encoder/data_test/ref0.pt')
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs
        ...
        def normalize_tensor_shape(tensor, attention_mask=None):
            """Normalize tensor shape by removing batch dimensions and handling different formats."""
            if tensor is None:
                return None
            
            # Handle position_ids FIRST (before general tensor handling)  
            if len(tensor.shape) == 3 and tensor.shape[0] == 3:
                # Handle position_ids with multiple conversations: (3, batch, seq_len)
                if attention_mask is not None and tensor.shape[1] > 1:
                    # For position IDs with batched format, we need special handling
                    # Flatten the last two dimensions: (3, batch, seq_len) -> (3, batch*seq_len)
                    flattened_tensor = tensor.view(tensor.shape[0], -1)
                    
                    # Apply attention mask to each position dimension
                    if len(attention_mask.shape) == 2:
                        attention_mask_flat = attention_mask.view(-1)
                    else:
                        attention_mask_flat = attention_mask
                        
                    if attention_mask_flat.shape[0] == flattened_tensor.shape[1]:
                        valid_indices = attention_mask_flat.bool()
                        # Apply mask to each dimension (x, y, z positions)
                        masked_tensor = flattened_tensor[:, valid_indices]
                        return masked_tensor
                
                # If no masking needed, handle single batch case
                if tensor.shape[1] == 1:
                    # Handle position_ids format: (3, 1, seq_len) -> (3, seq_len)
            ...
            if len(tensor.shape) >= 3:
                ...
                if tensor.shape[0] > 1:  # Multiple conversations
                    ...
                    if attention_mask is not None:
                        ...
                        if len(attention_mask.shape) == 2:  # [batch, seq_len]
                    ...
                    # Single conversation: (1, seq_len, dim) -> (seq_len, dim)
                    squeezed_tensor = tensor.squeeze(0)
                    
                    # If attention mask is provided for single conversation
                    if attention_mask is not None:
                        if len(attention_mask.shape) == 2:  # [1, seq_len] -> [seq_len]
                            attention_mask_flat = attention_mask.squeeze(0)
                        else:
                            attention_mask_flat = attention_mask
                            
                        # Remove padding tokens
                        if len(attention_mask_flat) == squeezed_tensor.shape[0]:
                            valid_indices = attention_mask_flat.bool()
                            squeezed_tensor = squeezed_tensor[valid_indices]
        ...
        norm_inputs_embeds = normalize_tensor_shape(inputs_embeds, attention_mask)
        ...
        norm_position_ids = normalize_tensor_shape(position_ids, attention_mask)
        ...
        norm_hidden_states = normalize_tensor_shape(hidden_states, attention_mask)
